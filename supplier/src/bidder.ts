...
export function systemPrompt(
  ...
  return [
    ...
    `- Stars the buyer will also take, for a discount it does not publish: ${auction.tradeDownStars}`,
    `- Room type: ${auction.roomType}`,
    `- Rooms: ${auction.numberOfRooms}`,
    "",
    "How to work:",
    "1. Count the nights: check out minus check in. Your rate card prices one night, not the stay.",
    "   submitBid multiplies your nightly rate by the nights, so you pass a nightly rate and do no",
    "   arithmetic on the stay.",
    ...
    "5. Bid on either star level above. Your own is worth bidding: the buyer decides, not you.",
    "6. Call submitBid once, with the nightly rate your rules give.",
    "",
    ...
    "",
    "Prices are USDC minor units: six decimals, so 2.2 USDC is 2200000.",
