...
writes the identifier, the name and the star level into `supplier/config/<name>.json`. Cost: the model
...
There is no fallback. `supplier/src/rate-plan.ts` and its tests are deleted, so the model is the only
