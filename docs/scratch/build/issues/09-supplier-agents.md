...
```
supplier/src/
  config.ts     zod schema and loader for supplier/config/<name>.json
...
`supplier/config/<name>.json` is committed: `relayUrl`, `rpcUrl`, `sealedAuction`, `priceRange`,
`model`, `effort`.
...
- [x] `supplier/src/rate-plan.ts` and `supplier/test/rate-plan.test.ts` are gone.
...
LiteAPI catalogue once and writes it into `supplier/config/<name>.json`, so the agent holds one tool
...
`pnpm --filter @perdiem/supplier test`: 14 pass, 0 fail.
