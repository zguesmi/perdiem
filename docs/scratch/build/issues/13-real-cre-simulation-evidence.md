...
the logs in `docs/evidence/`. This ticket is the moment that swap happens, and it is not optional:
...
- [ ] One full `cre workflow simulate` run is saved to `docs/evidence/simulate-<date>.log` and is
      committed. The `!docs/evidence/*.log` rule is already in `.gitignore`, so confirm with
