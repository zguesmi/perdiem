...
- [ ] `location` and `radiusMeters` appear nowhere in `shared/`, `workflow/`, `onchain/` or
      `agents/`.
- [ ] `distanceMeters` appears nowhere in those four directories, and the EIP-712 `Bid` type has ten
      fields.
- [ ] `BID_TYPEHASH` in the contract equals the hash of the new type string. One test states it.
- [ ] The Policy `version` is still `1`.
- [ ] Eligibility is city, checkin, checkout, roomType, numberOfRooms, price and stars. No distance.
