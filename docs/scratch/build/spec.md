...
resolved, and the schema it settled is in `docs/spec.md`.
...
- The Enclave pipeline around scoring is `docs/spec.md` step 7 and belongs to no ticket. That
  pipeline decrypts, checks each signature, checks each commitment, and builds the Bids Root. Ticket
  06 is the pure scoring function only.
