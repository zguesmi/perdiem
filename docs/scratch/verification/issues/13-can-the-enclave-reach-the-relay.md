...
- Whether plain `http` is permitted or the call must be `https`, which would mean a certificate for a
  local service.
...
passed into the handler as an argument by the workflow, which weakens nothing about secrecy but makes
the workflow nodes hold ciphertext they currently never see.
