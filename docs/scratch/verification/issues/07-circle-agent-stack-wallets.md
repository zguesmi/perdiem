...
2. That wallet can sign and broadcast a contract call, not only a transfer. `commit(auctionId,
   commitment)` is the call that matters, because it also needs an ERC-20 approval first.
